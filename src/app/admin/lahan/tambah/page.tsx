...
export default function TambahLahanPage() {
  ...
  useEffect(() => {
    async function fetchMitra() {
      ...
      if (Array.isArray(data)) {
        setMitraList(data.filter((u: User) => u.role === "mitra"));
      }
    }
    fetchMitra();
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!form.nama || !form.luas_hektar || !form.user_id) {
      setError("Nama, luas hektar, dan mitra wajib diisi");
      setLoading(false);
      return;
    }
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      setError(data.error || "Gagal menambah lahan");
    }
    setLoading(false);
  }
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Tambah Lahan</h1>
